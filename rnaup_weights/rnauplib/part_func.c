...
static char rcsid[]  = "$Id: part_func.c,v 1.29 2008/02/23 10:10:49 ivo Exp $";
...
PUBLIC  double justLoopEnergy(int u1, int u2, int type, int type2,
           short si1, short sj1, short sp1, short sq1);
...
PUBLIC float pf_fold(char *sequence, char *structure)
{
  ...
  // char fileName[100];
	...
	{
		for (ia=ja-TURN-1; ia>=1; ia--) 
		{
			int ij = iindx[ia]-ja;
			double ali_energy = (-log(q[ij])-(ja-ia+1)*log(pf_scale))*(temperature+K0)*GASCONST/1000.0;
			double kt2 = (temperature+K0)*GASCONST/1000.0;
			double ali_pf = exp(ali_energy / kt2 * -1);
			//fprintf(fpali, "%d\t%d\t%d\t%d\t%d\t%f\t%f\t%f\n", ia, ja, iindx[ia], iindx[ia]-ja, (ja-ia+1), q[ij],ali_energy,ali_pf);
			// fprintf(fpali, "%d\t%d\t%f\n", ia-1, ja-1, ali_pf);
  ...
  for (j=TURN+2;j<=n; j++) 
  {
	...
	for (k=i+1; k<=MIN(i+MAXLOOP+1,j-TURN-2); k++) {
	  ...
	  for (l=MAX(k+TURN+1,j-1-MAXLOOP+u1); l<j; l++) {
        ...
        double xx = expLoopEnergy(u1, j-l-1, type, type_2,
          S1[i+1], S1[j-1], S1[k-1], S1[l+1]);
        double xy = justLoopEnergy(u1, j-l-1, type, type_2,
          S1[i+1], S1[j-1], S1[k-1], S1[l+1]);
	...
	if ((j<n) || circ) qbt1 *= expdangle3[type][S1[j+1]];
	  ...
	  double ali_energy = (-log(q[ij])-(j-i+1)*log(1.987750))*(temperature+K0)*GASCONST/1000.0;
	  double kt2 = (temperature+K0)*GASCONST/1000.0;
	  double ali_pf = exp(ali_energy / kt2 * -1);
	  //fprintf(fpali, "%d\t%d\t%f\t%f\t%f\n", i, j, q[ij],ali_energy,ali_pf);
...
PRIVATE void scale_pf_params(unsigned int length)
{
  ...
  for (i=0; i<=NBPAIRS; i++)
    for (j=0; j<=NBPAIRS; j++) {
      ...
      //printf("Tmeasure = %f\n", Tmeasure);
      //printf("SE: i = %d, j = %d, GT = %f, TT = %f, kT = %f  \n", i, j, GT,TT, kT);
  ...
  for (i=0; i<=NBPAIRS; i++)
    ...
    // printf("BP: %d %d %d %d -> %f,  GT = %f,  kt = %f\n", i,j,k,l,expint11[i][j][k][l], GT, kT);
	}
  /* interior 2x1 loops */
  for (i=0; i<=NBPAIRS; i++)
    for (j=0; j<=NBPAIRS; j++)
...
/*
u1 is the length (i-k-1) of loop side on RNA 1 (num of nucs... so stacking pair has length 0)
u2 is the length (l-j-1) of loop side on RNA 2 (num of nucs... so stacking pair has length 0)
Pair:
    E   H
    v   ^
    F > G
...
s's are nucleotide type numbers, not letters
*/
...
type and type2 are the base pair nums
...
How to reverse this to get energy (GT):

  log (expstack[i][j] ) -> -GT*10/kT
  therefore
  - kT * log (expstack[i][j] ) -> 10*GT

   10*GT is in joules, so divide by 1000 to get kJ



*/

   // printf("In justLoopEnergy\n");
  ...
  //Reverse stacking now:
  double GT10 = -1 * kT * log(expstack[type][type2]);
  double GT = GT10 / 1000;
  // printf("GT = %f\n", GT);
