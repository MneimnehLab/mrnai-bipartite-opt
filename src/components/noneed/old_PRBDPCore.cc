...
void PRBDPCore::atStepH(int theIndex, int * indices)
{
    ...
    for(i=0;i<totalLevels-1;i++)
    {
        ...
        int botSeq = elemToRNAmap[i+1][indices[i+1]];
        ...
        for(w=1;w<=25;w++)
        {
            for(w2=1;w2<=25;w2++)
            {
                ...
                */
                ...
                indices[i+1] = compBotSeqStart + cutoffs[botSeq];
