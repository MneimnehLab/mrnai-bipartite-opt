/* Last changed Time-stamp: <2009-05-19 16:41:39 ivo> */
/* Last changed Time-stamp: <2009-05-19 16:41:39 ivo> */
/*
...
#include "duplex.h"
/*@unused@*/
static char rcsid[] = "$Id: part_func_up.c,v 1.4 2008/07/04 14:27:36 ivo Exp $";
...
PUBLIC double justLoopEnergy(int u1, int u2, int type, int type2, short si1, short sj1, short sp1, short sq1);
...
PUBLIC pu_contrib *pf_unstru(char *sequence, int w)
{
  ...
  for (i=1; i<=n; i++) {
	...
	for (i = 1; i < n; i++)
    {
        for (j = i; j < MIN((i + w), n); j++)
        {
			...
			//fprintf(fp_pu, "%d\t%d\t%f\t%f\n", i,j+1, sum_stru, dG_us);
  ...
  for (i=1; i<n; i++) {
    for (j=i; j < MIN((i+w),n); j++) {
	  ...
	  //ALI:: print probability of structure
	  //printf("%d\t%d\t%f\t%f\t%f\t%f\n", i,j,pu_stru->H[i][j-i], pu_stru->I[i][j-i], pu_stru->M[i][j-i], pu_stru->E[i][j-i]);
...
typedef struct btrk_tuple
{
  int type; //0: split, 1: loop
  int p;
  int q;
...
void backTrackEnergy(int k, int l, int i, int j, btrk_tuple **** backtrackMaxEnergyMtrx, int depth, int n2)
{
    int d=0;
    //for(d=0;d<depth;d++) printf("   ");
    //printf("k = %d, i = %d, l = %d, j = %d  \n", k, i, l, j);
    btrk_tuple bt = backtrackMaxEnergyMtrx[k][l][i][j];
    if(bt.type == 0)
    {
        //for(d=0;d<depth;d++) printf("   ");printf("type = 0, split at %d, %d\n", bt.p, bt.q);
        /*
        now need to backtrack on (k, bt.p, l, bt.q) and (bt.p, i, bt.q, j)
        */
        if (bt.p > k && bt.q < l)
            backTrackEnergy(k, l, bt.p, bt.q, backtrackMaxEnergyMtrx, depth+1, n2);

        if(bt.p < i && bt.q > j)
            backTrackEnergy(bt.p, bt.q, i, j, backtrackMaxEnergyMtrx, depth+1, n2);
    }
    else if(bt.type == 1)
    {
        //for(d=0;d<depth;d++) printf("   ");
...
PUBLIC interact *pf_interact(const char *s1, const char *s2, pu_contrib *p_c, pu_contrib *p_c2, int w, char *cstruc, int incr3, int incr5) 
{
  ...
  printf("pf_interact()\n");
#endif
    double RT = ((temperature + K0) * GASCONST / 1000.0);
    ...
    char fileName[50];
    sprintf(fileName, "Gint_holder_%d_%d.out", n1, n2);
    //fp = fopen(fileName, "w");

    // printf("AA123\n");

    // Initialize up rnaWins for pairwise subadditive testing

    //printf("initing rnawins for n1=%d, n2=%d \n", n1, n2);  
    rnaWins = (double ****) space(sizeof (double ***) * (n1 + 1));

    for (i = 0; i <= n1; i++) {
        rnaWins[i] = (double ***) space(sizeof (double **) * (n2 + 1));
        for (j = 0; j <= n2; j++) {
            rnaWins[i][j] = (double **) space(sizeof (double *) * (26));
            for (k = 0; k <= 25; k++) {
                rnaWins[i][j][k] = (double *) space(sizeof (double) * (26));
                int k2 = 0;
                for (k2 = 0; k2 <= 25; k2++) {
    ...
    // printf("Created rnaWins array\n");



    int nmax = MAX(n1, n2);
    double **** maxEnergyMtrx = (double ****) space(sizeof (double ***) * (nmax + 1));

    for (i = 0; i <= nmax; i++) {
        maxEnergyMtrx[i] = (double ***) space(sizeof (double **) * (nmax + 1));
        for (j = 0; j <= nmax; j++) {
            maxEnergyMtrx[i][j] = (double **) space(sizeof (double *) * (nmax+1));
            for (k = 0; k <= nmax; k++) {
                maxEnergyMtrx[i][j][k] = (double *) space(sizeof (double) * (nmax+1));
                int k2 = 0;
                for (k2 = 0; k2 <= nmax; k2++) {
                    maxEnergyMtrx[i][j][k][k2] = INF_1;
                    // printf("init i=%d, j=%d, k=%d, k2=%d, INF_1=%f\n", i,j,k,k2,INF_1);
                }
            }
        }
    }
    ...
    btrk_tuple **** backtrackMaxEnergyMtrx = (btrk_tuple ****) space(sizeof (btrk_tuple ***) * (nmax + 1));

    for (i = 0; i <= nmax; i++) {
        backtrackMaxEnergyMtrx[i] = (btrk_tuple ***) space(sizeof (btrk_tuple **) * (nmax + 1));
        for (j = 0; j <= nmax; j++) {
            backtrackMaxEnergyMtrx[i][j] = (btrk_tuple **) space(sizeof (btrk_tuple *) * (nmax+1));
            for (k = 0; k <= nmax; k++) {
                backtrackMaxEnergyMtrx[i][j][k] = (btrk_tuple *) space(sizeof (btrk_tuple) * (nmax+1));
                int k2 = 0;
                for (k2 = 0; k2 <= nmax; k2++) 
                {
                    btrk_tuple t = {-1,-1,-1};
                    backtrackMaxEnergyMtrx[i][j][k][k2] = t;
                }
            }
        }
    }
  ...
  so (i,j) is western base pair and (k,l) is eastern
  hence (k,l) is type2, and (i,j) is type
    */

    /*
    To compute min energy of region bounded by (k,l) and (i,j)

    E(k,l,i,j) = min{ loopEng(k,l,i,j), or min_a,b {E(k,l,a,b) + E(a+1,b+1,i,j)} }
    */
    ...
    for (i = 1; i <= n1; i++) 
    {
        ...
        for (j = n2; j > 0; j--) 
        {
           // printf("  j = %d\n", j);
            ...
            if (i > 1) 
              qint_4[i][j][0][0] *= Pf->expdangle5[type][S1[i - 1]];

            if (j < n2 && dangles) 
              qint_4[i][j][0][0] *= Pf->expdangle3[type][SS2[j + 1]];
            else if (type > 2) 
              qint_4[i][j][0][0] *= Pf->expTermAU;

            rev_d = 1.;
            
            if (i < n1 && dangles) 
              rev_d *= Pf->expdangle3[rtype[type]][S1[i + 1]];
            else if (type > 2) 
              rev_d *= Pf->expTermAU;
            
            if (j > 1) 
              rev_d *= Pf->expdangle5[rtype[type]][SS2[j - 1]];
            ...
            for (k = i - 1; k > end_k && k > 0; k--) 
            {
                ...
                for (l = j + 1; l < end_l && l <= n2; l++) 
                {
                    ...
                    // printf("iter:  k = %d, i = %d   ,   l = %d, j = %d\n", k, i, l, j);
                    //printf("iter:  i = %d, j = %d, k = %d, k = %d\n", i, j, k, l);
                    ...
                    */
                    ...
                    if (i - k + l - j - 2 <= MAXLOOP) 
                    {
                        ...
                        { 
                          /* don't violate constrains */
                            ...
                            double E2 = expLoopEnergy(i - k - 1, l - j - 1, type2, rtype[type],
                                    S1[k + 1], SS2[l - 1], S1[i - 1], SS2[j + 1]);



                            loopEnergy = justLoopEnergy(i - k - 1, l - j - 1, type2, rtype[type],
                                    S1[k + 1], SS2[l - 1], S1[i - 1], SS2[j + 1]); /* add *scale[u1+u2+2] */

#if ALI_DEBUG == 1
                            printf("E = %f,  E2 = %f, loopEnergy = %f, scale[..] = %f\n", E, E2, loopEnergy, scale[i - k + l - j]);
#endif
                    ...
                    // We have two choices: treat whole region as one interior loop, 
                    // or split at (p,q) and add energies of both sides

                            
                    // printf("chars: %d\t%d\t%d\t%d\n", S1[k + 1], SS2[l - 1], S1[i - 1], SS2[j + 1]);
                    
                    //printf("map: %d\t%d\t%d\t%d\t%f\n", i - k - 1, l - j - 1, type2, rtype[type], E);
                    //printf("i = %d, revj = %d, len1 = %d, len2 = %d, E = %f\n", i, revJ_, i - k - 1, l - j - 1, loopEnergy);
                    //printf("iter:  k = %d, i = %d   ,   l = %d, j = %d\n", k, i, l, j);
                    //printf("\n");
                    ...
                    */
                    ...
                    //p = {k+1 to i-1} increasing
                    //q = {l-1 to j+1} decreasing
                    int p,q, p_arg = 0, q_arg = 0;
                    double minE = INF_1;
                    for(p=k+1; p<i; p++)
                    {
                        for(q=l-1; q>j; q--)
                        {
                            ...
                            double E1 = maxEnergyMtrx[k][l][p][q];
                            double E2 = maxEnergyMtrx[p][q][i][j];

                            // printf("k=%d, i=%d, l=%d, j=%d, p=%d, q=%d E1 = %f,  E2 = %f,  minE = %f\n", 
                                        // k,i,l,j,p,q,E1, E2, minE);
                            if(minE > E1 + E2)
                            {
                    ...
                    if(minE < loopEnergy) //Split is better
                    {
                        bkrec.type = 0;
                        bkrec.p = p_arg;
                        bkrec.q = q_arg;
                        //minE is correct val already
                        // printf("go with current minE\n");
                    }
                    else  //Use loop 
                    {
                        bkrec.type = 1;
                        minE = loopEnergy;
                        // printf("Use loop energy B\n");
                    }

                    maxEnergyMtrx[k][l][i][j] = minE;
                    backtrackMaxEnergyMtrx[k][l][i][j] = bkrec;
                    // printf("minE = %f\n", minE);

                  //int revJ = n2 - j + 1;
                  //int revL = n2 - l + 1;

                    //printf("1. Setting maxEnergyMtrx[%d][%d][%d][%d] = %f  , with type %d  \n", k, l, i, j, minE, bkrec.type);
                    ...
                    if (G_is > -99)
                        rnaWins[i][l][a][b] = G_is;
                    else

                        rnaWins[i][l][a][b] = 995;

                    double myval = (-log(intt) - const_scale)*(const_T);

                    
                    //rnaWins[i][l][a][b] = G_is;
                    //if(doFp) fprintf(fp, "%d\t%d\t%d\t%d\t%f \n",k,i,j,l,G_is);
                    int revJ = n2 - j + 1;
                    int revL = n2 - l + 1;

#if ALI_DEBUG == 1
                    printf("%d\t%d\t%d\t%d\n", k , i , revL , revJ );
                    printf("-log(intt) = %f\t intt = %e\t const_scale = %f\t const_T = %f\t scalew = %e\t rev_d = %f\n\t myval = %f\t qint_4[i][j][a][b] = %f\n", 
                          -log(intt), intt, const_scale, const_T,scalew, rev_d, myval, qint_4[i][j][a][b]);
                    ...
                    //g_minus_gint + myval = G_is ;
                    //g_minus_gint + log(interaction) = G_is ; where myval = log(interaction) 

                    // Note: myval -> G_interaction
                    if(isinf(intt) != 0 || isinf(tt) != 0 )
                      continue;
                    
                    double Ev = maxEnergyMtrx[k][l][i][j];

                    double g_minus_gint = G_is - myval;   // equivalent to E1 + E2
                    ...
                    intt = qint_4[i][j][a][b] * scalew*rev_d;
                    G_is  = (-log(tt)   - const_scale)*(const_T);
                    myval = (-log(intt) - const_scale)*(const_T);
                    g_minus_gint = G_is - myval;

                    E1 + E2  ==should_be==  g_minus_gint = G_is - G_interaction
                    -->
                    -RT*log(myP1) - RT*log(myP2)   =sb=    (-log(tt) - const_scale)*(const_T)  -  (-log(intt) - const_scale)*(const_T)
                    -RT*log(myP1) - RT*log(myP2)   =sb=    (-log(tt) - const_scale)*(RT)  -  (-log(intt) - const_scale)*(RT)
                    -RT*log(myP1) - RT*log(myP2)   =sb=    -RT*(log(tt) + const_scale)  -  (-RT)*(log(intt) + const_scale)
                    -RT*log(myP1) - RT*log(myP2)   =sb=    -RT*[(log(tt) + const_scale)  -  (log(intt) + const_scale)]
                    log(myP1) + log(myP2)          =sb=    [log(tt) + const_scale - log(intt) - const_scale]
                    log(myP1) + log(myP2)          =sb=    [log(tt) - log(intt)]
                    log(myP1) + log(myP2) + log(intt)     =sb=    log(tt)
                    log(myP1 * myP2 * intt)     =sb=    log(tt)
                    YES

                    */

                    // printf("x = %f, y = %f, x+y = %f,   a = %f, b = %f, a-b = %f\n", 
                    //         log(myP1), log(myP2), log(myP1) + log(myP2), log(tt), log(intt), log(tt) - log(intt));
                    ...
                    //fprintf(fpNew, "%d\t%d\t%d\t%d\t%f\n",k-1,i-1,revL-1,revJ-1,G_is);
                }
            }
            Z += temp;
            /* int_ik */
